...
class TwitterSocialMediaService {
    ...
    /**
     * Integrates with extended Nimble bootstrap process, sets up Twitter Environment
    ...
    public void nimbleInit() {
        ...
        if (!twitterMediaService) {
            ...
            twitterMediaService.baseProfileUrl = new SocialUrl(location: grailsApplication.config.nimble.twitter.profileurl, altText: grailsApplication.config.nimble.twitter.profilealttext)
    ...
    def create(def profile, def userID) {
        if(twitterMediaService){
            ...
            twitterAccount.profile = new SocialUrl(location: location, altText: twitterMediaService.baseProfileUrl.altText)
